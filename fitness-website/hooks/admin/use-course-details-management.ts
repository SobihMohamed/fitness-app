...
interface CourseDetailsReturn
  extends CourseDetailsState, CourseDetailsActions {}

export function useCourseDetailsManagement(
  courseId: string,
): CourseDetailsReturn {
  ...
  const loadModulesAndChapters = useCallback(async () => {
    ...
    try {
      ...
      // If modules/chapters endpoints return 404 for empty state, treat as normal.
  ...
  const updateModule = useCallback(
    async (moduleId: string, formData: ModuleFormData) => {
      try {
        setLoading((prev) => ({ ...prev, saving: true }));
  ...
  const createChapter = useCallback(
    async (moduleId: string, formData: ChapterFormData) => {
      try {
        setLoading((prev) => ({ ...prev, saving: true }));
  ...
  const updateChapter = useCallback(
    async (chapterId: string, formData: ChapterFormData) => {
      try {
        setLoading((prev) => ({ ...prev, saving: true }));
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Errors are already surfaced in the called functions; avoid duplicate toasts.
      } finally {
