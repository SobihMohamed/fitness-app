...
export function useDashboardData(user: any) {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        API_CONFIG.USER_FUNCTIONS.orders.getMyOrders,
        { headers },
      );
      ...
      const normalized = (Array.isArray(ordersData) ? ordersData : []).map(
        (raw: any) => {
          const order = {
            id: String(raw.id || raw.order_id || raw._id || Math.random()),
            total_price: Number(
              raw.total_price || raw.net_total || raw.total || raw.amount || 0,
            ),
            status: String(
              raw.status || raw.status_name || raw.state || "unknown",
            ),
            created_at: String(
              raw.created_at ||
                raw.purchase_date ||
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const headers = getAuthHeaders();
      const response = await axios.get(
        API_CONFIG.USER_FUNCTIONS.notifications.getAll,
        { headers },
      );
      ...
      const normalized = (
        ...
        .map((raw: any) => {
          ...
          const notification = {
            id: String(
              raw?.id ?? raw?.notification_id ?? raw?._id ?? Math.random(),
            ),
            title: String(
              raw?.title ??
                raw?.message_title ??
  ...
  const loadSubscribedCourses = useCallback(async () => {
    try {
      ...
      const normalized = (Array.isArray(coursesData) ? coursesData : []).map(
        (raw: any) => {
          const course = {
            ...
            enrolled_at: String(
              raw.enrolled_at || raw.created_at || new Date().toISOString(),
            ),
  ...
  const loadRequests = useCallback(async () => {
    try {
      ...
      try {
        ...
        const trainingData =
          trainingResponse.data?.requests ||
          trainingResponse.data?.data ||
      ...
      try {
        ...
        try {
          courseResponse = await axios.post(
            API_CONFIG.USER_FUNCTIONS.requests.courses.getMyRequests,
            {},
            { headers },
          );
        } catch (postError: any) {
          if (postError.response?.status === 404) {
            ...
            // Method not allowed — try GET as fallback
            courseResponse = await axios.get(
              API_CONFIG.USER_FUNCTIONS.requests.courses.getMyRequests,
              { headers },
            );
          } else {
  ...
  const updateProfile = useCallback(
    async (data: ProfileData & { password?: string }) => {
      const headers = getAuthHeaders();
      const payload: any = {
        name: data.name?.trim() ?? "",
  ...
  const deleteNotification = useCallback(
    async (id: string) => {
      try {
        const headers = getAuthHeaders();
        await axios.delete(API_CONFIG.USER_FUNCTIONS.notifications.delete(id), {
          headers,
        });
