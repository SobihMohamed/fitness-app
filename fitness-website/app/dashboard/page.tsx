...
const StatsCard = dynamic(
  () => import("@/components/dashboard/StatsCard").then((m) => m.StatsCard),
  {
    loading: () => (
...
const ProfileSection = dynamic(
  () =>
    import("@/components/dashboard/ProfileSection").then(
      (m) => m.ProfileSection,
    ),
  {
    loading: () => (
...
const OrdersSection = dynamic(
  () =>
    import("@/components/dashboard/OrdersSection").then((m) => m.OrdersSection),
  {
    loading: () => (
...
const CoursesSection = dynamic(
  () =>
    import("@/components/dashboard/CoursesSection").then(
      (m) => m.CoursesSection,
    ),
  {
    loading: () => (
...
const StatsSkeleton = dynamic(
  () =>
    import("@/components/dashboard/LoadingSkeleton").then(
      (m) => m.StatsSkeleton,
    ),
  {
    loading: () => (
  ...
  const loadOrderDetails = useCallback(
    async (orderId: string) => {
      try {
            ...
            try {
              ...
              if (cached) {
                ...
                setOrderDetailsError(
                  "Showing cached order data. Full details unavailable.",
                );
                return;
              }
              throw err2;
        ...
        const raw =
          body?.order || body?.data?.order || body?.data || body || {};
        ...
        if (
          ...
          itemsSource &&
          Array.isArray(itemsSource.data)
        ) {
        ...
        const normalized: any = {
          ...
          status: String(
            (raw as any)?.status || (raw as any)?.status_name || "unknown",
          ),
          total_price: Number(
          ...
          payment_method:
            ...
            (raw as any)?.method ||
            (raw as any)?.gateway ||
            (raw as any)?.order?.payment_method ||
        ...
        if (cached) {
          ...
          if (
            ...
            Array.isArray((cached as any).products)
          ) {
        ...
        if (
          ...
          ((raw as any)?.product_id || (raw as any)?.productId)
        ) {
          const pid = String(
            (raw as any)?.product_id || (raw as any)?.productId,
          );
        ...
        try {
          ...
          if (missing.length > 0) {
            const uniqueIds: string[] = Array.from(
              new Set(missing.map((m: any) => String(m.product_id))),
            ) as string[];
            const results = await Promise.allSettled(
              uniqueIds.map(async (pid) => {
                ...
                const res = await axios.get(
                  API_CONFIG.USER_FUNCTIONS.products.getById(pid),
            ...
            results.forEach((r: any) => {
              if (r.status === "fulfilled" && r.value?.pid)
                nameMap.set(String(r.value.pid), r.value.name || "");
            });
            normalized.items = normalized.items.map((it: any) => {
              if (
                (!it.name || it.name === "Item") &&
                it.product_id &&
                nameMap.get(String(it.product_id))
              ) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600 text-lg md:text-xl max-w-2xl mx-auto">
            Track your fitness journey, manage your orders, and stay updated
            with your progress
          </p>
        </div>
                  ...
                  {orderDetailsLoading ? (
                    ...
                    <div className="space-y-6">
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Badge
                          className={`${getStatusColor(orderDetails.status)} border text-sm px-3 py-1`}
                        >
                          {orderDetails.status}
                        </Badge>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {typeof orderDetails.discount_value !== "undefined" &&
                          Number(orderDetails.discount_value) > 0 && (
