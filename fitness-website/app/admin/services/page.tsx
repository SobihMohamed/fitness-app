...
export default function ServicesManagement() {
    ...
    const initializeData = async () => {
      try {
        await Promise.all([
          (async () => {
            try {
              const res = await fetch(`${API_BASE}/AdminServiceCategories/getAll`, {
              ...
              if (res.ok) {
                ...
                showErrorToast("Failed to load service categories")
              }
            } catch (err) {
              showErrorToast("Network error while loading service categories")
            }
          })(),
          fetchServices(),
        ])
  ...
  const addCategory = async () => {
    if (!newCategoryName.trim()) {
      showErrorToast("Service category name is required")
      return
    }
    ...
    if (existingCategory) {
      showErrorToast("A service category with this name already exists")
      return
    }

    try {
      setIsSubmitting(true)
      const res = await fetch(`${API_BASE}/AdminServiceCategories/addCategory`, {
        method: "PUT",
        headers: getAuthHeaders(),
      ...
      if (res.ok) {
        ...
        setNewCategoryName("")
        const refreshRes = await fetch(`${API_BASE}/AdminServiceCategories/getAll`, {
          headers: getAuthHeaders(),
        })
        const refreshData = await refreshRes.json()
        if (refreshRes.ok) {
          setServiceCategories(refreshData.data || [])
        }
      } else {
        showErrorToast(data.message || "Failed to add service category")
      }
    } catch (err) {
      showErrorToast("Network error while adding service category")
    } finally {
  ...
  const updateCategory = async () => {
    ...
    const currentCategory = serviceCategories.find((cat) => cat.category_id === editingCategoryId)
    ...
    try {
      setIsSubmitting(true)
      const res = await fetch(`${API_BASE}/AdminServiceCategories/updateCategory/${editingCategoryId}`, {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        showErrorToast(data.message || `Failed to update service category`)
      }
    } catch (err) {
      showErrorToast("Network error while updating service category")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const confirmDeleteCategory = (id: string) => {
    const categoryName = serviceCategories.find((cat) => cat.category_id === id)?.name || "this category"
    const servicesInCategory = services.filter((s) => s.category_id === id)
    if (servicesInCategory.length > 0) {
      showErrorToast(
        `Cannot delete category "${categoryName}". It contains ${servicesInCategory.length} service(s). Please remove or reassign the services first.`,
      )
      return
    }
  ...
  const deleteCategory = async () => {
    ...
    try {
      setIsSubmitting(true)
      const res = await fetch(`${API_BASE}/AdminServiceCategories/deleteCategory/${deleteTarget.id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      })
      const data = await res.json()
      if (res.ok) {
        ...
        setServiceCategories(serviceCategories.filter((cat) => cat.category_id !== deleteTarget.id))
      } else {
  ...
  const generateCategories = () => {
    const existingNames = serviceCategories.map((cat) => cat.name.toLowerCase())
    const newCategories = SERVICE_CATEGORY_SUGGESTIONS.filter(
  ...
  const fetchServices = async () => {
    await withLoading("services", async () => {
      try {
        const res = await fetch(`${API_BASE}/AdminServices/getAll`, {
          headers: getAuthHeaders(),
        })
        if (!res.ok) {
        ...
        showErrorToast(err.message || "Network error while loading services")
      }
    })
  }
  ...
  const filteredServices = services.filter(
    (s: Service) =>
      ...
      (selectedCategory === "all" || s.category_id?.toString() === selectedCategory),
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.price || !formData.duration) {
      showErrorToast("Service name, price, and duration are required")
      return
    }
    if (!formData.category_id) {
      showErrorToast("Please select a service category")
      return
    }
    ...
    try {
      setIsSubmitting(true)
      const endpoint = editingService ? `updateService/${editingService.service_id}` : "addService"
      const res = await fetch(`${API_BASE}/AdminServices/${endpoint}`, {
        method: "POST",
        headers: {
      ...
      showErrorToast(err.message || "Network error while saving service")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleEdit = (service: Service) => {
    ...
    setFormData({
      ...
      description: service.description || "",
      features: service.features || "",
      is_active: service.is_active?.toString() || "",
  ...
  const confirmDeleteService = (id: string) => {
    ...
    setDeleteTarget({ type: "service", id, name: serviceName })
    setShowDeleteConfirm(true)
  }

  const deleteService = async () => {
    if (!deleteTarget || deleteTarget.type !== "service") return
    try {
      setIsSubmitting(true)
      const res = await fetch(`${API_BASE}/AdminServices/deleteService/${deleteTarget.id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      })
      const result = await parseResponse(res)
      ...
      showErrorToast(err.message || "Network error while deleting service")
    } finally {
      setIsSubmitting(false)
      setShowDeleteConfirm(false)
      setDeleteTarget(null)
  ...
  const totalCategoryPages = Math.ceil(serviceCategories.length / categoriesPerPage)
  const paginatedCategories = serviceCategories.slice(
      ...
      <div className="space-y-8 p-6 bg-gradient-to-br from-slate-50 to-white min-h-screen">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-600 mt-3 text-lg">Manage your fitness services and training programs with ease</p>
          </div>
          <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="p-3 bg-indigo-200 rounded-full">
                  <Dumbbell className="h-8 w-8 text-indigo-700" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-lg bg-gradient-to-br from-emerald-50 to-emerald-100 hover:shadow-xl transition-all duration-200">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-emerald-700 mb-1">Active Services</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-purple-900">{serviceCategories.length}</p>
                </div>
        ...
        <Card className="border-0 shadow-lg">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-slate-600">
              Create and manage service categories for better organization
            </CardDescription>
          </CardHeader>
            ...
            {showCategoryGenerator && (
              <div className="bg-white rounded-lg p-4 border border-indigo-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {SERVICE_CATEGORY_SUGGESTIONS.filter(
                    (suggestion) =>
                      !serviceCategories.some((cat) => cat.name.toLowerCase() === suggestion.toLowerCase()),
                  )
                    .slice(0, 6)
        ...
        <Card className="border-0 shadow-lg">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-slate-600">Find services by name or filter by category</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {(searchTerm || selectedCategory !== "all") && (
              <div className="mt-4 flex items-center gap-3">
                <span className="text-sm text-slate-600">Found {filteredServices.length} services</span>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => {
                ...
                <TableBody>
                  {paginatedServices.map((service: Service) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <div>
                          <div className="font-semibold text-slate-900 mb-1">{service.name}</div>
                          {service.description && (
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-xs">
                          {serviceCategories.find((cat) => cat.category_id === service.category_id)?.name ||
                            service.category_id}
                        </Badge>
                      </TableCell>
                      <TableCell className="font-semibold text-indigo-600">{service.price}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge
                          variant={service.is_active === "1" ? "default" : "secondary"}
                          className={`text-xs ${
                            service.is_active === "1"
              ...
              {filteredServices.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <h3 className="text-xl font-semibold text-slate-900 mb-2">
                    {searchTerm || selectedCategory !== "all" ? "No services found" : "No services yet"}
                  </h3>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  rows={3}
                  placeholder="Enter service features (one per line or comma-separated)..."
              ...
              {/* Image Upload */}
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Service Image</label>
                <Input
                  type="file"
                  accept="image/*"
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {deleteTarget?.type === "service" && " The associated image will also be removed."}
              </DialogDescription>
            </DialogHeader>
