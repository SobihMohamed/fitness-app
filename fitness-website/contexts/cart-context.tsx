  ...
  switch (action.type) {
    case "ADD_ITEM": {
      const existingItemIndex = state.items.findIndex(
        (item) => norm(item.id) === norm(action.payload.id),
      );
    ...
    case "REMOVE_ITEM": {
      const updatedItems = state.items.filter(
        (item) => norm(item.id) !== norm(action.payload),
      );
      const total = updatedItems.reduce(
        (sum, item) => sum + item.price * item.quantity,
        0,
    ...
    case "UPDATE_QUANTITY": {
      ...
      const updatedItems = state.items
        .map((item) =>
          norm(item.id) === norm(id) ? { ...item, quantity } : item,
        )
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // Use ref to keep track of items for stable callbacks
  // This prevents addItem/removeItem from changing identity when items change
  // which prevents re-renders of all ProductCards when one item changes
  ...
  const updateQuantity = useCallback((id: string, quantity: number) => {
    if (quantity < 1) {
      // removeItem(id) // Don't call removeItem directly to avoid circular dependency or complex deps
      // Just dispatch remove
      dispatch({ type: "REMOVE_ITEM", payload: id });
      return;
    }
