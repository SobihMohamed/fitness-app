...
//Removed useMemo for lightweight computations → fewer hooks per render.

// Reduced component re-renders by avoiding inline functions that change each render.

// Simplified error fallback (1x1 image only once).

// No redundant string operations in the render path.

// Slightly fewer DOM elements (some nested divs flattened).

// Still fully SSR-safe and client-optimized.
...
const BlogCard = React.memo<BlogCardProps>(
  ({ blog, categoryName, featured = false }) => {
    const [imageOk, setImageOk] = React.useState(true);
    const displayCategory = categoryName || blog.categoryName || "General";

    // Lightweight date parsing
    const safeDate = (() => {
      const raw = blog.createdAt as string | undefined;
      if (!raw) return "Recently";

      const formats = [raw, raw.replace(" ", "T")];
    ...
    const imageSrc = clientBlogApi.getImageUrl(blog.featuredImage);
    ...
    return (
      <Card
        className={`overflow-hidden group bg-white border border-gray-100 hover:border-gray-200 
          rounded-2xl transition-all duration-300 shadow-md hover:shadow-xl hover:-translate-y-1 
          ${featured ? "mb-16" : ""}`}
      >
        <div className={featured ? "relative" : "md:flex"}>
          {/* IMAGE SECTION */}
          <div className={featured ? "h-64" : "md:w-2/5"}>
            ...
            >
              {imageOk && imageSrc ? (
                ...
                <BookOpen
                  className={`${featured ? "h-16 w-16" : "h-12 w-12"} text-primary`}
                />
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-black/5 via-transparent to-transparent" />
            </div>
            ...
            <div className="md:w-3/5 p-6">
              ...
              <div className="flex gap-3">
                <Link href={`/blog/${blog.id}`} className="flex-1">
                  <Button
                    variant="outline"
                    className="w-full border-gray-200 text-blue-700 font-medium py-2 bg-white"
                  >
                    Read Article
                    <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
                  </Button>
                ...
                {blog.videoUrl && (
                  <Link href={`/blog/${blog.id}?watch=1#video`}>
                    <Button className="font-medium py-2 px-6 bg-primary text-white">
                      Watch
                      <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
                    </Button>
                  </Link>
