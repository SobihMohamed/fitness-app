...
import { useCourseRequests } from "@/hooks/client/use-course-requests";
import {
  ShoppingCart, Heart, Share2, CheckCircle, Users, Eye,
} from "lucide-react";
...
const CourseEnrollmentSection = React.memo(({ course }: Props) => {
  const { user } = useAuth();
  const { makeAuthenticatedRequest } = useUserApi();
  const { getCourseRequestStatus, canEnroll, getEnrollmentButtonState, refetch } = useCourseRequests();
  ...
  const [form, setForm] = useState({ gender: "", job: "", age: "", promoCode: "" });
  const [submitting, setSubmitting] = useState(false);

  const originalTotal = course.price || 0;
  const discountValue = form.promoCode.trim().toUpperCase() === "DISCOUNT10" ? 10 : 0;
  const netTotal = Math.max(0, originalTotal - discountValue);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!form.gender || !form.job || !form.age) return toast.error("Please fill all required fields");
    ...
    try {
      ...
      if (data?.status === "success") {
        toast.success("Enrollment request submitted!");
        setDialogOpen(false);
        setForm({ gender: "", job: "", age: "", promoCode: "" });
        refetch();
  ...
  }, [user, form, makeAuthenticatedRequest, refetch, course.course_id, originalTotal, discountValue, netTotal]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-gray-100 shadow-lg sticky top-24">
        <CardHeader className="text-center pb-4">
          <div className="text-4xl font-bold mb-2">{formatNumber(course.price)} EGP</div>
          <Badge variant="secondary" className="bg-green-100 text-green-800">Best Value</Badge>
        </CardHeader>

        <CardContent className="space-y-4">
          <Button
            onClick={buttonState.disabled ? undefined : handleOpenEnroll}
            disabled={buttonState.disabled}
            className={buttonState.className}
            size="lg"
          >
              ...
              {status !== "approved" && (
                <p className="text-muted-foreground mt-2 text-xs">
                  {status === "pending"
                    ...
                    : "Your previous request was cancelled. You can submit a new one."}
                </p>
              )}
            </div>
          )}
          ...
          <div className="space-y-3 text-sm">
            {[
              ["Students Enrolled", formatNumber(course.students_count || 0)],
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="job">Job</Label>
                <Input id="job" value={form.job} onChange={handleChange} placeholder="Your job" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="age">Age</Label>
                <Input id="age" type="number" value={form.age} onChange={handleChange} min={1} />
              </div>
              <div>
                <Label htmlFor="promoCode">Promo Code</Label>
                <Input id="promoCode" value={form.promoCode} onChange={handleChange} placeholder="Optional" />
              </div>
            </div>
