...
const ProductCard = React.memo<ProductCardProps>(
  ({
    ...
    const stockBadge = React.useMemo(() => {
      if (isOutOfStock)
        return (
          <Badge variant="destructive" className="text-white font-semibold px-4 py-2 text-sm uppercase tracking-wider">
            Out of Stock
          </Badge>
        );
      ...
      <Card className="group overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 bg-white rounded-xl border-0">
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Link href={`/products/${product.product_id}`} className="col-span-2">
              <Button className="w-full font-medium bg-blue-600 hover:bg-blue-700 text-white transition-all duration-300" size="lg">
                View Details
              </Button>
            </Link>
            ...
            <ProtectedAction onAction={handleAddToCart}>
              <Button
                ...
                className="w-full font-medium bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-50 transition-all duration-300"
                size="lg"
              >
                <ShoppingCart className="w-5 h-5 mr-2" />
