...
export const ProductDetailsDialog = React.memo<ProductDetailsDialogProps>(
  ({
    ...
    const handleReplaceFile = useCallback(
      (e: React.ChangeEvent<HTMLInputElement>, oldPath: string) => {
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto overflow-x-hidden p-0 rounded-2xl border-none shadow-2xl bg-white">
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                <img
                  src={
                    product.main_image_url
                      ? withCacheBuster(
                          getProxyImageUrl(product.main_image_url),
                          cacheBuster,
                        )
                      : "/placeholder.svg"
                  }
              ...
              <div className="space-y-4 min-w-0">
                <h3 className="text-2xl font-bold text-slate-900 break-words whitespace-pre-wrap line-clamp-3">
                  {product.name}
                </h3>
                <div className="space-y-2">
                  ...
                  {product.description && (
                    <div>
                      ...
                      <p className="text-slate-700 mt-1 whitespace-pre-wrap break-words max-h-48 overflow-auto pr-2">
                        {product.description}
                      </p>
                    </div>
                  )}
            ...
            <div className="flex justify-end items-center pt-6 mt-6 border-t border-slate-200">
              <Button
                ...
                variant="outline"
                onClick={onClose}
                className="mr-auto hover:bg-slate-100 hover:text-slate-900 border-slate-300 text-slate-800 font-medium px-5"
              >
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className="group hover:bg-indigo-50 hover:border-indigo-200 hover:text-slate-900 text-slate-800 transition-all duration-150"
                >
                  <Edit3 className="h-4 w-4 text-indigo-600 group-hover:text-indigo-700 mr-2" />
                  Edit Product
                </Button>
              </div>
            </div>
