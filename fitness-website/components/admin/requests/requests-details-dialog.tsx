...
// Helper components
const DetailItem = React.memo<{ label: string; value: any }>(
  ({ label, value }) => (
...
const TextDetailItem = React.memo<{ label: string; value: any }>(
  ({ label, value }) => (
    <div className="space-y-2">
      <div className="text-sm text-slate-500">{label}</div>
                      ...
                      {detailsData.order_items?.length > 0 ? (
                        detailsData.order_items.map(
                          (item: any, idx: number) => (
                            <li
                              key={idx}
                              className="bg-white p-3 rounded-md border border-slate-200 shadow-sm"
                            >
                              ...
                              <div className="mt-2 flex flex-wrap gap-4 text-sm text-slate-600">
                                ...
                                <span>
                                  ...
                                  <span className="font-medium">
                                    {(() => {
                                      const d = Number(
                                        detailsData.discount_value ??
                                          detailsData.order?.discount_value,
                                      );
                                      return isNaN(d)
                                        ? "-"
                                        : `${d.toFixed(2)}%`;
                                ...
                                {(() => {
                                  const up = Number(
                                    detailsProduct?.price ||
                                      detailsProduct?.unit_price ||
                                      detailsProduct?.price_value ||
                                      NaN,
                                  );
                                  return isNaN(up) ? "-" : formatCurrency(up);
                                })()}
                              </span>
                            </span>
                            <span>
